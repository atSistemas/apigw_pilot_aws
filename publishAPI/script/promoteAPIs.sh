...
aws cognito-idp describe-user-pool --user-pool-id $userPoolId --profile pilot-$1 --region $region > "tmp/cognito-idp-describe-user-pool-$1-tmp.json"
...
aws lambda get-function --function-name $lambdaRouter --profile pilot-$1 --region $region > "tmp/lambda-get-function-$1-tmp.json"
lambdaRouterArn=$(jq -r '.Configuration.FunctionArn' "tmp/lambda-get-function-$1-tmp.json")
lambdaProxyUri="arn:aws:apigateway:$region:lambda:path/2015-03-31/functions/"$lambdaRouterArn"/invocations"
echo "lambdaProxyUri: $lambdaProxyUri"

#Obtenermos el account ID.
aws sts get-caller-identity --profile pilot-$1 --region $region > "tmp/aws-sts-get-caller-identity-$1-tmp.json"
  ...
  do  
    ...
    do 
      ...
      then
        ...
        #Se compone fichero json con los scopes a dar de alta en el servidor de recursos punto 9.
        ...
        #Se compone array con los metodos y path necesarios para conceder permisos en el punto 10.
...
echo "5. Se reemplaza userPoolARN en el fichero openapi-$1.json."
...
echo "6. Se codifica el fichero openapi-$1.json en base64:"
jq -c '.' "tmp/openapi-$1.json" | base64 > "tmp/openapi-$1-base64.txt"
echo "$PWD/tmp/openapi-$1-base64.txt \n"

echo "7. Se descarga de AWS los APIs publicados en el API Gateway."
aws apigateway get-rest-apis --profile pilot-$1 --region $region > "tmp/aws-apigateway-get-rest-apis-$1-tmp.json"
echo "$PWD/tmp/aws-apigateway-get-rest-apis-$1-tmp.json \n"

echo "8. Se comprueba si el API a publicar existe o no en AWS API Gateway."
api_id=$(jq -r --arg apiname "$apiname" '.items[] | select(.name==$apiname).id' "tmp/aws-apigateway-get-rest-apis-$1-tmp.json")

if [ -z "$api_id" ]
then
    echo " 8.1 No se ha encontrado ningún api publicado en API Gateway bajo el nombre: $apiname."
    echo " 8.2 Se procede a importar el nuevo api: $apiname, a partir del fichero openapi-'$1'-base64.txt."
    aws apigateway import-rest-api --fail-on-warnings --body 'file://./tmp/openapi-'$1'-base64.txt' --profile pilot-$1 --region $region > "tmp/aws-apigateway-import-rest-api-$1-tmp.json"
    echo " 8.3 Se obtiene el identificador del API importado."
    api_id=$(jq -r '.id' "tmp/aws-apigateway-import-rest-api-$1-tmp.json")
    echo " rest-api-id: $api_id"
    echo " 8.4 Se procede a desplegar el API y a crear un stage."
    aws apigateway create-deployment --rest-api-id $api_id --stage-name $stageName --stage-description "$apiname" --description "$apiname" --region $region --profile pilot-$1 > "tmp/aws-apigateway-create-deployment-$1-tmp.json"
    deployment_id=$(jq -r '.id' "tmp/aws-apigateway-create-deployment-$1-tmp.json")
    echo " rest-api-deployment-id: $deployment_id"
else
    echo " 8.1 Se ha encontrado el identificador: $api_id para el $apiname."
    echo " 8.2 Se procede a subir los cambios al API ya existente a partir del fichero openapi-'$1'-base64.txt."
    aws apigateway put-rest-api --rest-api-id $api_id --no-fail-on-warnings --mode overwrite --body 'file://./tmp/openapi-'$1'-base64.txt' --profile pilot-$1 --region $region > "tmp/aws-apigateway-put-rest-api-$1-tmp.json"
    echo " $PWD/tmp/aws-apigateway-put-rest-api-$1-tmp.json"
    echo " 8.3 Se procede a desplegar el API y actualizar stage."
    aws apigateway create-deployment --rest-api-id $api_id --stage-name $stageName --stage-description "$apiname" --description "$apiname" --region $region --profile pilot-$1 > "tmp/aws-apigateway-create-deployment-$1-tmp.json"
    ...
    echo " 8.4 Se recogen los id de despliegues."
    aws apigateway get-deployments --rest-api-id $api_id --region $region --profile pilot-$1 > "tmp/aws-apigateway-get-deployments-$1-tmp.json"
    echo " $PWD/tmp/aws-apigateway-get-deployments-$1-tmp.json"
    echo " 8.5 Se eliminan los despliegues previos:"
    ...
    do
      ...
      then
        ...
        aws apigateway delete-deployment --rest-api-id $api_id --deployment-id $deploymentId --profile pilot-$1 --region $region
      fi
    done
fi
echo "\n"
...
echo "9. Se comprueba si existe el servidor de recursos:"
aws cognito-idp describe-resource-server --user-pool-id $userPoolId --identifier $resourceServerName --profile pilot-$1 --region $region > "tmp/cognito-idp-describe-resource-server-$1-tmp.json"
...
then
    ...
    aws cognito-idp update-resource-server --user-pool-id $userPoolId --identifier $resourceServerName --name $resourceServerName --scopes 'file://./tmp/resourceScopes-'$1'-tmp.json' --profile pilot-$1 --region $region > "tmp/cognito-idp-update-resource-server-$1-tmp.json"
    ...
    aws cognito-idp create-resource-server --user-pool-id $userPoolId --identifier $resourceServerName --name $resourceServerName --scopes 'file://./tmp/resourceScopes-'$1'-tmp.json' --profile pilot-$1 --region $region > "tmp/cognito-idp-create-resource-server-$1-tmp.json"
...
echo "10. Se procede a conceder permisos para invocar a la lambda proxy:"

aws lambda get-policy --function-name $lambdaRouter  --profile pilot-$1 --region $region | jq -rc '.Policy' | jq '.Statement' > "tmp/lambda-get-policy-statement-$1-tmp.json"
...
then
    ...
    do
      ...
      aws lambda remove-permission --function-name $lambdaRouter --statement-id $statementId --profile pilot-$1 --region $region
    done
...
do
  ...
  aws lambda add-permission --function-name $lambdaRouter --action lambda:InvokeFunction --statement-id APIGatewayInvokePermission$timeMillis --principal apigateway.amazonaws.com --source-arn $addPermissionArn --profile pilot-$1 --region $region > "tmp/lambda-add-permission-$1-$timeMillis-tmp.json"
